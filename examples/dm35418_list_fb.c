/**
    ...
    $Id: dm35418_list_fb.c 141531 2024-03-06 21:05:25Z lfrankenfield $
*/
...
int main(int argument_count, char **arguments)
{
	...
	struct DM35418_Function_Block my_func_block;
	char errMsg[300];
	uint8_t rev_num;
	...
	result = DM35418_Gbc_Get_Revision(board, &rev_num);
	DM35418_Check_Result(result, "Error getting board revision number");

	result = DM35418_Gbc_Get_Fpga_Build(board,
						&fpga_num);
	DM35418_Check_Result(result, "Error getting FPGA build number.");

	result = DM35418_Gbc_Get_Pdp_Number(board,
						&pdp_num);
	DM35418_Check_Result(result, "Error getting PDP number.");
	...
	fb_num = 0;
	while ((fb_num < DM35418_MAX_FB) && (errno != ERANGE)) {
		result = DM35418_Function_Block_Open(board,
		...
		if (errno != ERANGE) {
			...
			sprintf(errMsg, "Could not open function block %d.",
				fb_num);
			DM35418_Check_Result(result, errMsg);

			switch (my_func_block.type) {

			case DM35418_FUNC_BLOCK_ADC:
				printf
			...
			case DM35418_FUNC_BLOCK_DIO:
				printf
				    ("  FB%d: DIO (Type 0x%x Subtype %d), with %d DMA Channels (%d buffers each)\n",
				     ...
				     my_func_block.num_dma_buffers);
				break;
			case DM35418_FUNC_BLOCK_SYNCBUS:
				printf
				    ("  FB%d: Syncbus Module (Type 0x%x Subtype %d), with %d DMA Channels (%d buffers each)\n",
				     my_func_block.fb_num,
				     my_func_block.type,
				     my_func_block.sub_type,
				     my_func_block.num_dma_channels,
				     my_func_block.num_dma_buffers);
				break;
			case DM35418_FUNC_BLOCK_TEMPERATURE_SENSOR:
				printf
				    ("  FB%d: Temp. Sensor Module (Type 0x%x Subtype %d), with %d DMA Channels (%d buffers each)\n",
				     ...
				     my_func_block.num_dma_buffers);
				break;
			case DM35418_FUNC_BLOCK_INVALID:
				// Do nothing
				break;
			default:
