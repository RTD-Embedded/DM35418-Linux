/**
    ...
    @verbatim
        ...
        To see the output data, connect an oscilloscope to the DACx.0
        pin, where x is the DAC you select.
    ...
    $Id: dm35418_dac.c 141531 2024-03-06 21:05:25Z lfrankenfield $
*/
...
int main(int argument_count, char **arguments)
{
	...
	DM35418_Check_Result(result, "Error setting output range.");

	result = DM35418_Dac_Reset(board, &my_dac);

	DM35418_Check_Result(result, "Error stopping DAC");
	...
	result = DM35418_Dac_Volts_To_Conv(DM35418_DAC_RNG_BIPOLAR_5V, voltage,
					   &conv_value);

	DM35418_Check_Result(result, "Error converting voltage to conversion");
	...
	printf("Sample: %6d\tVoltage: %3.2f\r", conv_value, voltage);
	...
	while (keypress != 'q') {
		...
		time_difference = DM35418_Get_Time_Diff(temp_clock, next_press);
		if (time_difference < 38000L) {
			...
			increment = (((int)
				      DM35418_Get_Time_Diff(next_press,
							    start_pressing)) +
				     1) / 50000;
		...
		if (keypress == 'i') {
			if (conv_value <= (0x7FFF - increment)) {
		...
		if (keypress == '0') {
			result =
			    DM35418_Dac_Volts_To_Conv
			    (DM35418_DAC_RNG_BIPOLAR_5V, 0, &conv_value);
		...
		if (keypress == '1') {
			result =
			    DM35418_Dac_Volts_To_Conv
			    (DM35418_DAC_RNG_BIPOLAR_5V, 1, &conv_value);
		...
		if (keypress == '2') {
			result =
			    DM35418_Dac_Volts_To_Conv
			    (DM35418_DAC_RNG_BIPOLAR_5V, 2, &conv_value);
		...
		if (keypress == '3') {
			result =
			    DM35418_Dac_Volts_To_Conv
			    (DM35418_DAC_RNG_BIPOLAR_5V, 3, &conv_value);
		...
		if (keypress == '4') {
			result =
			    DM35418_Dac_Volts_To_Conv
			    (DM35418_DAC_RNG_BIPOLAR_5V, 4, &conv_value);
		...
		if (keypress == '5') {
			...
			result =
			    DM35418_Dac_Volts_To_Conv
			    (DM35418_DAC_RNG_BIPOLAR_5V, 4.999847412f,
			     &conv_value);
		...
		if (keypress == '%') {
			result =
			    DM35418_Dac_Volts_To_Conv
			    (DM35418_DAC_RNG_BIPOLAR_5V, -5, &conv_value);

			DM35418_Check_Result(result,
				     "Error converting voltage to conversion");
		}

		result = DM35418_Dac_Set_Last_Conversion(board,
							 &my_dac,
							 CHANNEL_0,
							 0, conv_value);
		DM35418_Check_Result(result, "Error setting last conversion");
		...
		result = DM35418_Dac_Get_Last_Conversion(board,
							 &my_dac,
							 CHANNEL_0,
							 &marker, &conv_value);
		...
		result = DM35418_Dac_Conv_To_Volts(DM35418_DAC_RNG_BIPOLAR_5V,
						   conv_value, &voltage);
		...
		printf("Sample: %6d \tVoltage: %3.2f    \r", conv_value,
		       voltage);
